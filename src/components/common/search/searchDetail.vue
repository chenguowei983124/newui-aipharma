<template>
  <div class="pt-2 rounded-lg md:rounded-none">
    <div class="space-y-2">
            ...
            <div class="flex flex-row">
              ...
              <div class="w-20 md:w-20">
                <label class="inline-flex items-center justify-end">
                  ...
                  />
                  <span class="ml-1 text-xs md:text-mxss">コメント</span>
                </label>
              </div>
            </div>
            <div class="flex flex-row">
              <div class="w-28 md:w-30">
                ...
                <label class="inline-flex items-center justify-end">
                  <input
                    type="checkbox"
                    class="checkboxCss"
                    :checked="$store.getters.getCheckAddFileName"
                    @change="onChangeCheckAddFileName"
                  />
                  <span class="ml-1 text-xs md:text-mxss">添付ファイル名</span>
                </label>
              </div>
              <div class="w-20 md:w-20">
                <!-- 投稿者 -->
                <label class="inline-flex items-center justify-end">
                  <input
                    type="checkbox"
                    class="checkboxCss"
                    :checked="$store.getters.getCheckContributor"
                    @change="onChangeCheckContributor"
                  />
                  <span class="ml-1 text-xs md:text-mxss">投稿者</span>
                </label>
              </div>
              <div class="w-20 md:w-28">
                ...
                <label class="inline-flex items-center justify-end">
                  ...
                  />
                  <span class="ml-1 text-xs md:text-mxss">最終編集者</span>
                </label>
              </div>
            </div>
            <div class="flex flex-row">
              ...
              <div class="w-28 md:w-20">
                <label class="inline-flex items-center justify-end">
                  ...
                  />
                  <span class="ml-1 text-xs md:text-mxss">施設名</span>
                </label>
              </div>
              <div class="w-20 md:w-16">
                ...
                <label class="inline-flex items-center justify-end">
                  ...
                  />
                  <span class="ml-1 text-xs md:text-mxss">備考</span>
                </label>
              </div>
            </div>
          </div>
        ...
        <div class="px-2 md:px-0">
          ...
          >
            ...
            <template v-slot:option="{ option }">
              <div class="w-full">
                # {{ option.label }}
                <div class="float-right" v-if="option.count !== undefined">
                  {{ option.count }}件
                </div>
              </div>
            </template>
      ...
      <!-- 三行目、四行目、五行目 -->
      <div
        class="space-y-2 bg-backgroundMainSearch mx-2 md:mx-0 font-NotoSansJp"
        ref="resetSelect"
      ...
      >
        ...
        <div class="hidden md:block mid:block">
          ...
          >
            <div>詳細条件</div>
            <triangle-down-svg
              :class="[isOrgDetailClick ? 'transform rotate-180' : '']"
              fill="#ffffff"
              stroke="#ffffff"
              class="h-3 w-3"
            ></triangle-down-svg>
      ...
      <!-- 七行目　-->
      <div class="bg-searchResultTitle rounded-b-lg block md:hidden mid:hidden">
        <div
...
export default {
      ...
      for (let index = 0; index < this.orgOrgTagslist.length; index++) {
        ...
        if (
          ...
          for (let i = 0; i < selectedItem.length; i++) {
            if (selectedItem[i].value == this.orgOrgTagslist[index].value) {
              storeExistFlg = true
            }
          }
          if (!storeExistFlg) {
  ...
  methods: {
    ...
    onChangeCheckNote() {
      this.$store.dispatch('setCheckNote', !this.$store.getters.getCheckNote)
    },
